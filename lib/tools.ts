...
export const toolDefinitions = {
	...
	replace_area: tool({
		...
		inputSchema: z.object({
			from_cell: z.string().describe(
				'Top-left cell coordinate (e.g., A1)',
			),
			to_cell: z
				.string()
...
export function toolReplaceArea(
	...
	if (fromColIndex > toColIndex) {
		return {
			success: false,
			error:
				'from_cell column must be less than or equal to to_cell column',
		}
	}
	...
	if (newAreaData.length > expectedRows) {
		return {
			success: false,
			error:
				`values has ${newAreaData.length} rows, but expected at most ${expectedRows} (from ${fromCell} to ${toCell})`,
		}
	}
	...
	for (let i = 0; i < newAreaData.length; i++) {
		if (newAreaData[i].length > expectedCols) {
			return {
				success: false,
				error: `Row ${i + 1} in values has ${
					newAreaData[i].length
				} columns, but expected at most ${expectedCols} (from ${fromCell} to ${toCell})`,
			}
		}
...
// Factory function to apply provider-specific options to tools
export function createTools<T extends Record<string, any>>(
	toolsWithExecute: T,
	...
	if (provider === 'anthropic') {
		// Add anthropic cache control to all tools
	...
	if (provider === 'openrouter') {
		// Add openrouter cache control to all tools
		return Object.fromEntries(
			Object.entries(toolsWithExecute).map(([name, tool]) => [
				name,
				{
	...
	// For other providers, return tools as-is
	return toolsWithExecute
}
