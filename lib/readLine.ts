...
function enableWindowsANSI() {
	if (Deno.build.os === "windows") {
		try {
			...
			// This enables ANSI escape sequences in Windows Console
			...
			// Silently fail if we can't enable ANSI
			// Modern Windows Terminal has it enabled by default
		}
	}
}
...
export async function readLine(prompt: string): Promise<string | null> {
	...
	const redrawLine = async () => {
		...
		await Deno.stdout.write(
			encoder.encode(
				`${afterCursor}\x1b[K${remaining > 0 ? `\x1b[${remaining}D` : ""}`,
			),
		);
	};
	...
	try {
		...
		while (true) {
			...
			if (code === 27 && n >= 3 && buf[1] === 91) {
				...
				if (escCode === 72 || (escCode === 49 && n >= 4 && buf[3] === 126)) {
					if (cursorPos > 0) {
				...
				if (escCode === 70 || (escCode === 52 && n >= 4 && buf[3] === 126)) {
			...
			if (code === 27 && n >= 2 && (buf[1] === 127 || buf[1] === 8)) {
				if (cursorPos > 0) {
					...
					if (match) {
						...
						buffer =
							buffer.slice(0, cursorPos - deleteCount) +
							buffer.slice(cursorPos);
						await Deno.stdout.write(encoder.encode(`\x1b[${deleteCount}D`));
